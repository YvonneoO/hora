...
class AllegroHandHora(VecTask):
    ...
    def reset_idx(self, env_ids):
        ...
        for n_s in range(num_scales):
            ...
            self.root_state_tensor[self.object_indices[s_ids], :7] = sampled_pose[:, self.num_allegro_hand_dofs:]
            self.root_state_tensor[self.object_indices[s_ids], 7:13] = 0
            pos = sampled_pose[:, :self.num_allegro_hand_dofs]
    ...
    def compute_observations(self):
        ...
        self.obs_buf_lag_history[at_reset_env_ids, :, self.num_allegro_hand_dofs:2 * self.num_allegro_hand_dofs] = self.allegro_hand_dof_pos[at_reset_env_ids].unsqueeze(1)
    ...
    def post_physics_step(self):
        ...
        self.reset_buf[:] = 0
        self._refresh_gym()
        # --- dense-taxel logging hook: log poses+forces for every hand body and the object ---
        import os as _os
        _tl = _os.environ.get('TACTILE_LOG', '')
        if _tl:
            if not hasattr(self, '_rbs_buf'):
                self._rbs_buf = []; self._tac_buf = []; self._jnt_buf = []; self._wz_buf = []
            _n = self.num_allegro_hand_bodies + 1
            _rbs = self.rigid_body_states[0, :_n, :7].detach().cpu().numpy()
            _tac = torch.linalg.norm(self.contact_forces[0, :_n, :], dim=-1).detach().cpu().numpy()
            self._rbs_buf.append(_rbs)
            self._tac_buf.append(_tac)
            self._jnt_buf.append(self.allegro_hand_dof_pos[0].detach().cpu().numpy())
            self._wz_buf.append(float(self.rigid_body_states[0, self.num_allegro_hand_bodies, 12].cpu()))
            if len(self._tac_buf) % 20 == 0:
                np.savez(_tl, rbs=np.array(self._rbs_buf), tactile=np.array(self._tac_buf),
                         joints=np.array(self._jnt_buf), objangvel_z=np.array(self._wz_buf),
                         num_hand_bodies=self.num_allegro_hand_bodies)
    ...
    def _setup_object_info(self, o_config):
        ...
        self.asset_files_dict = {
            ...
            'cyl': 'assets/cylinder.urdf',
            'box0': 'assets/cuboid/default/0000.urdf',
        }
    ...
    def _init_object_pose(self):
        ...
        _hp = _e.get('handPos', [0, 0, 0.5])
        allegro_hand_start_pose.p = gymapi.Vec3(_hp[0], _hp[1], _hp[2])
        _rpy = _e.get('handRPY', None)
        if _rpy is not None:
            allegro_hand_start_pose.r = (
                gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), _rpy[2])
                * gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 1, 0), _rpy[1])
                * gymapi.Quat.from_axis_angle(gymapi.Vec3(1, 0, 0), _rpy[0]))
        else:
        ...
        pose_dx, pose_dy, pose_dz = _e.get('objOffset', [-0.01, -0.05, 0.15])
        ...
        _oz = self.config['env'].get('objZ', None)
        if _oz is not None:
            object_z = _oz
        else:
